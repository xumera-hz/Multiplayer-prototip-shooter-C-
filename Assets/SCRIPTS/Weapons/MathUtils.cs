...
{
    public static Vector2 LineIntersection2D(Vector2 a1, Vector2 a2, Vector2 b1, Vector2 b2)
    {
        Vector2 point = Vector2.zero;

        float temp1 = (a1.x * a2.y - a1.y * a2.x);
        float temp2 = (b1.x * b2.y - b1.y * b2.x);
        float temp3 = ((a1.x - a2.x) * (b1.y - b2.y) - (a1.y - a2.y) * (b1.x - b2.x));
        if (temp3 < 1e-5f && temp3 > -1e-5f)
        {
            #region debug
#if UNITY_EDITOR
            Debug.LogWarning("Прямые практически параллельны!");
#endif
            #endregion
            return point;
        }

        point.x = (temp1 * (b1.x - b2.x) - (a1.x - a2.x) * temp2) / temp3;
        point.y = (temp1 * (b1.y - b2.y) - (a1.y - a2.y) * temp2) / temp3;
    ...
    public const float DEFAULT_DISTANCE_FIRE = 1f;//не делать меньше, близким или равным нулю
    public const float MIN_DISPERSION_FIRE = 1E-5f;

    public static Vector3 RandomVectorInsideCone2(float d)
    {
        Vector3 v3;
        if (d < MIN_DISPERSION_FIRE)
        {
            v3.x = v3.y = 0f;
            v3.z = 1f;
        }
        else
        {
            ...
            float z = DEFAULT_DISTANCE_FIRE / d;
            float magnitude = 1f / (float)System.Math.Sqrt(v2.x * v2.x + v2.y * v2.y + z * z);
            v3.z = z * magnitude;
            v3.x = v2.x * magnitude;
            v3.y = v2.y * magnitude;
        }
