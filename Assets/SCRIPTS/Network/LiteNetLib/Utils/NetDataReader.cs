...
namespace LiteNetLib.Utils
{
    public class NetDataReader
    {
        ...
        public byte[] GetCopyRemainingBytes()
        {
            byte[] outgoingData = new byte[AvailableBytes];
            Buffer.BlockCopy(_data, _position, outgoingData, 0, AvailableBytes);
            return outgoingData;
        ...
        System.Runtime.Serialization.IFormatter _BinaryFormatter;
        public T Deserializer<T>()
        {
            int count = GetInt();
            if (count <= 0 || (_position + count) >= _data.Length) return default(T);
            if (_BinaryFormatter == null) _BinaryFormatter = new System.Runtime.Serialization.Formatters.Binary.BinaryFormatter();
            T ReturnValue;
